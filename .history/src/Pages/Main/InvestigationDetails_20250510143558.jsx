    ...
    <div className="max-w-full mx-auto p-4 bg-gray-50 min-h-screen">
        ...
        <div className="bg-white rounded-sm shadow-sm border border-gray-200 p-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-[#00256c]/5 rounded-sm">
                <FileText className="text-[#00256c]" size={24} />
              </div>
              <div>
                <h1 className="text-xl font-medium text-gray-900">
                  Investigation in Respect of <span className='text-[#00256c]'>{investigation.taxpayers[0].trade_name}</span> having GSTIN <span className='text-[#00256c]'>{investigation.taxpayers[0].gstin}</span>
                </h1>
            ...
            <div className="flex gap-2">
              <button 
                className="p-2 text-gray-500 hover:text-[#00256c] hover:bg-[#00256c]/5 rounded-sm transition-colors" 
                title="Edit"
              >
                <Edit size={18} />
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white rounded-sm shadow-sm border border-gray-200">
                ...
                <div className="p-4">
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center justify-between gap-3 p-3 bg-gray-50/50 rounded-sm border border-gray-100">
                      <div className="flex items-center gap-3">
                        <Calendar className="text-[#00256c]" size={18} />
                        <div>
                          <p className="text-xs text-gray-500">Period From</p>
                          <p className="font-medium text-gray-900">{investigation.from_date ? new Date(investigation.from_date).toLocaleDateString() : '-'}</p>
                        </div>
                      </div>
                      <span className="text-gray-400">-</span> 
                      <div className="mr-4">
                        <p className="text-xs text-gray-500">Period To</p>
                        <p className="font-medium text-gray-900">{investigation.to_date ? new Date(investigation.to_date).toLocaleDateString() : '-'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 bg-gray-50/50 rounded-sm border border-gray-100">
                    ...
                    <div className="flex items-center gap-3 p-3 bg-gray-50/50 rounded-sm border border-gray-100">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-900">{investigation.source_name || '-'}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-sm shadow-sm border border-gray-200 overflow-hidden sticky top-4">
            <div className="p-3 border-b border-gray-100">
              <h2 className="text-lg font-medium text-[#00256c] flex items-center gap-2">
                <Plus size={20} className="text-[#00256c]" />
                Actions
              </h2>
            </div>
            <div className="p-2">
              {actionButtons.map((action, index) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 rounded-sm transition-colors text-sm ${
                    activePage === action.name.toLowerCase().replace(/\s+/g, '-')
                      ? 'bg-[#00256c]/5 text-[#00256c]'
