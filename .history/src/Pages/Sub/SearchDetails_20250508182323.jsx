...
import { Plus, ArrowLeft, FileText, User, Building, Phone, Edit, Trash2, X } from "lucide-react";
...
const SearchDetails = ({ fileNumber }) => {
  ...
  const fetchSearchOfficers = async () => {
    try {
      const data = await AxiosWrapper(
        "get",
        `investigation/search-officers/search/${fileNumber}/`
      );
      setSearchOfficers(data);
      setIsLoading(false);
    } catch (error) {
  ...
  const handleUpdateOfficer = async (e) => {
    ...
    try {
      ...
      // Refresh the list
      fetchSearchOfficers();
      toast.success("Search officer updated successfully");
    } catch (error) {
      console.error("Error updating search officer:", error);
      toast.error(error.message || "Failed to update search officer");
    }
  };
  ...
  const handleDeleteOfficer = async () => {
    ...
    try {
      ...
      // Refresh the list
      fetchSearchOfficers();
      toast.success("Search officer deleted successfully");
    } catch (error) {
      console.error("Error deleting search officer:", error);
      toast.error(error.message || "Failed to delete search officer");
      setShowDeleteConfirm(false);
    }
  };
  ...
  const DeleteConfirmationModal = () => (
    <div className="fixed inset-0 bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-xl">
        ...
        <p className="mb-6">
          Are you sure you want to delete this search officer? This action cannot be undone.
        </p>
