...
const InspectionDetails = ({ fileNumber }) => {
  ...
  const [showCompetencyDropdown, setShowCompetencyDropdown] = useState({
    add: false,
    edit: false
  });
  const competencyOptions = ["Superintended", "AC/BC", "ADC/JC"];
  ...
  // Modified version of the competency field for forms
  const CompetencyField = ({ value, onChange, formType }) => {
    return (
      <div className="relative">
        ...
        <div className="relative">
          <input
            ...
            value={value}
            readOnly
            onClick={() => setShowCompetencyDropdown(prev => ({ ...prev, [formType]: true }))}
            className="w-full border border-gray-300 rounded-md px-3 py-2 pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer"
            required
            autoComplete="off"
          ...
          <div 
            ...
            onClick={() => setShowCompetencyDropdown(prev => ({ ...prev, [formType]: !prev[formType] }))}
          >
            <ChevronDown size={18} className="text-gray-500" />
          </div>
        </div>
        {showCompetencyDropdown[formType] && (
          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto">
            {competencyOptions.map((option, index) => (
              <div 
                key={index}
                className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${value === option ? 'bg-blue-50 font-medium' : ''}`}
                onClick={() => {
                  onChange({ target: { name: 'issued_by', value: option } });
                  setShowCompetencyDropdown(prev => ({ ...prev, [formType]: false }));
                }}
              >
                {option}
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <form onSubmit={handleUpdateInspection} className="p-4">
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <CompetencyField value={formData.issued_by} onChange={handleChange} formType="edit" />
                 <InputBox
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <form onSubmit={handleAddInspection} className="p-4">
            <div className="space-y-6">
              ...
              {/* Purpose and DIN Section */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <CompetencyField value={formData.issued_by} onChange={handleChange} formType="add" />
