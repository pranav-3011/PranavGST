...
const SummonsDetails = ({ fileNumber }) => {
  ...
  // Add stable key to prevent complete re-rendering of the form
  const getFormKey = () => {
    if (selectedSummons && isEditing) {
      return `edit-summons-${selectedSummons.id}`;
    }
    return 'add-summons';
  };

  // Optimize the form component to use React.memo
  const MemoizedSummonsForm = React.memo(({ onSubmit, submitButtonText }) => (
