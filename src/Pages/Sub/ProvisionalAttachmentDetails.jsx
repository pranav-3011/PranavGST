      ...
      {formData.attachment_type === "other" && (
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <textarea
              name="other_attachment_details"
              value={formData.other_attachment_details || ""}
              onChange={handleChange}
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              rows="4"
            />
  ...
  const fetchAttachments = async () => {
    try {
      const data = await AxiosWrapper(
        "get",
        `investigation/provisional-attachments/investigation/${fileNumber}/`
      );
      setAttachments(data);
      setIsLoading(false);
    } catch (error) {
    ...
    try {
      ...
      if (isEditing) {
        setFormData({
          ...
          other_attachment_details: data.other_attachment_details || "",
        });
      }
    } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="p-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              {selectedAttachment.attachment_type === "other" && selectedAttachment.other_attachment_details && (
                <div className="col-span-2 flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                  <Tag className="text-gray-500 mt-1" size={18} />
                  <div>
                    <p className="text-sm text-gray-600">Other Attachment Details</p>
                    <p className="font-medium whitespace-pre-wrap">{selectedAttachment.other_attachment_details}</p>
                  </div>
                </div>
              )}
          ...
          >
            ...
            <div onClick={() => handleViewAction(attachment.id)} className="h-full">
              ...
              <div className="space-y-2">
                ...
                {attachment.attachment_type === "other" && attachment.other_attachment_details && (
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    <Tag size={16} />
                    <span className="truncate">{attachment.other_attachment_details || "-"}</span>
                  </div>
                )}
              </div>
            </div>
